        ...
        public bool InsertarFamiliaUsuario(FondoFamiliaEntity familiaUsuario)
        {
            using (var cnn = SqlLiteConexion.SimpleDbConnection())
            {
                int result = cnn.Execute(
                    @"INSERT INTO FONDOUSUARIO (N_ID, N_ID_USUARIO, V_NOMBRES, V_APELLIDOS, V_TIPO_IDENTIFICACION, V_NUMERO_IDENTIFICACION, F_FECHA_NACIMIENTO, V_TIPO_PARENTESCO, 
                      N_TELEFONO, V_TIPO_ACTIVIDAD, V_CUAL_OTRA)
                      VALUES(@N_ID, @N_ID_USUARIO, @V_NOMBRES, @V_APELLIDOS, @V_TIPO_IDENTIFICACION, @V_NUMERO_IDENTIFICACION, @F_FECHA_NACIMIENTO, @V_TIPO_PARENTESCO, 
                      @N_TELEFONO, @V_TIPO_ACTIVIDAD, @V_CUAL_OTRA)",
                    familiaUsuario);
                return result > 0;
            }
